    ...
    for i in range(rows):
        for j in range(cols):
            if chars[i][0] in tokens[j][0]:
                matrix[i][j] = (
                    0.001 * j
                )  # /len(tokens[j][0])#0.00001*tokens[j][1]#-math.log(tokens[j][1]*1.0/total_tokens)*1.0/len(tokens[j][0]) + 0.1/
            else:
                matrix[i][j] = 100  # 0
    return matrix
...
def write_vocab(tokens, pmatrix, chars, write_file_name, threshold=0.0001):
    ...
    for j in tqdm(range(len(pmatrix[0]))):
        ...
        for i in range(len(pmatrix)):
            if (
                pmatrix[i][j] != 0
                and pmatrix[i][j] * total_tokens > threshold * tokens[j][1]
            ):
                new_tokens[tokens[j][0]][chars[i][0]] = (
                    pmatrix[i][j] * total_tokens
                )  # * len(tokens[j][0])
    ...
    for token in new_tokens:
        ...
        itemlist = []
        if (
            len(new_tokens[token]) == 0
            and token.replace(" ", "") not in required_actions
        ):
            print(token, new_tokens[token])
            continue
        if token.strip() != "":
    ...
    with open(write_file_name, "w") as f:
        for item in vocab_tokens:
            f.write(item + "\n")
    print(
        "The vocabulary has been written into the given file. You can use this vocabulary to segment tokens in subword-nmt"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--target_file", default=None, help="path to a target file for translation"
    )
    parser.add_argument(
        "--token_candidate_file",
        default=None,
    ...
    parser.add_argument(
        "--tokenizer", default="subword-nmt", choices=["sentencepiece", "subword-nmt"]
    )
    parser.add_argument(
        "--size_file", default="size.txt", help="the size file stores the best size"
    )
